...
- 项目版本从 `0.4.0` 升级到 `0.5.0`。
- 本发布窗口新增文章分类归档与分类分页、文章 SEO/OG 图片元数据和分类筛选能力，属于向后兼容的 Minor 级功能更新。

### 🎉 Added

- 新增 `/articles/category/[category]` 分类归档和 `/articles/category/[category]/page/[page]` 分类分页路由，复用现有文章浏览、搜索和分页体验。
...
- 文章列表搜索、标签筛选和无限滚动现在支持分类条件，并在分类归档中保持当前路径。
...
- 项目版本从 `0.3.0` 升级到 `0.4.0`。
- 本发布窗口新增完整的浏览器本地工具箱、全站导航入口与异次元自动跃迁能力，属于向后兼容的 Minor 级功能更新。

### 🎉 Added

- 新增 `/tools` 工具箱页面与顶部导航入口，提供 JSON 格式化、时间戳转换、URL 编解码、Base64 转换、文本检查和颜色对比度共 6 个本地工具。
- 工具目录新增名称与关键词搜索、分类筛选、结果数量、空状态，以及宫格与列表展示模式切换。
- 全站顶部多功能区新增“异次元之旅”自动跃迁入口，使用简约单色行星轨道图标并在新窗口开启旅行。

### 🔄 Changed

- 工具箱图标统一适配项目的单色线性风格，列表模式补充浮起、位移与箭头反馈，并适配窄屏导航间距。
- 颜色与对比度工具集成 `react-colorful` 与 Radix Popover，将颜色选择器改为非模态弹出层，支持实时更新 HEX、RGB 与 WCAG 检查结果。
- 全站文本链接统一使用贴近文字基线的动态下划线，兼容鼠标悬停、键盘焦点与减少动画偏好。

### 🐛 Fixed

- 修复中文或包含空格的文章标签在静态路由与客户端跳转间编解码不一致导致的 `404`。
- 修正文本链接动态下划线距离文字过远的视觉问题。

### ✅ Verify

- `pnpm.cmd run typecheck`、`pnpm.cmd run lint`、`pnpm.cmd run build` 与 `git diff --check` 检查通过；生产构建成功生成 81 个静态页面。
- 构建仅保留 Browserslist 数据过期的非阻塞警告。
...
- 本次全站内容索引与全局搜索浮窗属于向后兼容的 Minor 级功能增强，未超过本窗口从 `0.2.3` 升至 `0.3.0` 已覆盖的级别，版本继续保持 `0.3.0`。
- 本次逐篇 RSS 收录控制、全站订阅自动发现和顶部全站工具区订阅入口属于向后兼容的 Minor 级功能增强，未超过本窗口已覆盖的 Minor 级别，版本继续保持 `0.3.0`。
- 本次首页 About 头像对齐与 RSS 顶部工具区迁移属于向后兼容的 Patch 级布局优化，未超过本窗口已覆盖的 Minor 级别，版本继续保持 `0.3.0`。
- 本次搜索命中高亮与句子感知摘要属于向后兼容的搜索体验增强，未超过本窗口从 `0.2.3` 升至 `0.3.0` 已覆盖的 Minor 级别，版本继续保持 `0.3.0`。
- 本次文章分页与无限滚动加载模式切换属于向后兼容的 Minor 级浏览能力增强，未超过本窗口从 `0.2.3` 升至 `0.3.0` 已覆盖的级别，版本继续保持 `0.3.0`。
- 本次默认无限滚动、浏览模式偏好记忆与文章列表回到顶部属于向后兼容的 Minor 级浏览体验增强，未超过本窗口从 `0.2.3` 升至 `0.3.0` 已覆盖的级别，版本继续保持 `0.3.0`。
...
- 文章列表新增构建期静态分页，第一页保留 `/articles`，后续生成 `/articles/page/N`，当前 25 篇文章对应 3 个归档页面。
- 新增静态标签首页及标签分页路由，文章详情与筛选区域统一链接到可索引的 `/articles/tag/[tag]` 路径。
- 新增按需加载的 `/site-search-index.json`，聚合首页常驻内容、文章归档入口、公开文章摘要与章节级正文索引。
- 全站顶部导航新增搜索浮窗，支持 `Ctrl/Cmd + K`、方向键选择、Enter 跳转、Escape 关闭、焦点恢复和移动端近全屏布局。
- 全站搜索浮窗与文章归档搜索新增结构化命中高亮，标题、章节、摘要及仅由元数据命中的关键词均以主题适配的红色 `<mark>` 显示。
- 文章归档新增“分页 / 无限滚动加载”Switch，模式通过 `view=infinite` 写入 URL，并支持自动滚动加载、手动加载更多、失败重试、分页回退以及搜索和标签切换时保留状态。
- 新增静态轻量文章摘要接口 `/article-index.json`，无限滚动模式仅按需加载文章列表元数据，不复用包含全文搜索文档的全站索引。
- 文章归档、标签列表与文章搜索结果新增右下角“回到顶部”按钮，在页面标题离开视口后显示，并支持 Tooltip、键盘焦点恢复、移动端安全区和减少动画偏好。
...
- 全站顶部导航新增 RSS 订阅入口，位于主题切换左侧，并提供 Tooltip、44px 触控目标、键盘焦点与紫色悬停反馈。
...
- 将首页 About 主标题收敛为“兮兮”，重写简介以明确前端开发者与 AIGC 站点定位，并恢复身份关键词的底部渐变背景标记。
- 收紧首页技术栈与服务信息的单行高度和纵向间隔，同时保持简介上边距及图标与文字的横向间距不变。
...
- 将文章仓库拆分为轻量元数据索引与按 slug 单篇正文读取，列表、标签、RSS、sitemap 和相邻文章不再重复提取全部文章的标题 AST。
- 将文章专用搜索器重构为可复用的全站搜索引擎，文章归档继续按文章范围筛选，全局浮窗则统一呈现页面、文章和章节结果。
- 搜索摘要改为由搜索层按句子与段落边界生成，优先保留完整命中句和适度相邻上下文，并移除展示层的两行强制截断。
- 将文章数量与当前页状态移至筛选区下方的结果工具栏，并统一分页、搜索、标签和无限滚动模式的 URL 生成；长列表文章项增加离屏渲染优化。
- 默认文章归档页只传递当前页摘要，搜索时再加载全量公开摘要索引；RSS 输出限制为最新 50 篇文章。
...
- 首页 About 调整为头像身份栏与介绍文本顶部对齐，收紧桌面列间距，并在窄屏下恢复单列左对齐。
- RSS 订阅入口从首页 About 身份栏迁移到全站顶部工具区，与主题切换共享 44px 透明圆形操作样式，同时删除首页专用 RSS 客户端组件和旧操作栏样式。
- 文章列表首次访问改为默认无限滚动加载，并使用版本化的 `article-browse-mode:v1` 本地偏好记忆用户选择；显式 `view` 参数和静态分页深链继续优先于本地偏好。
- 分页模式新增显式 `view=pagination` URL，普通无模式入口在客户端解析偏好后使用 History Replace 规范化地址，同时保留 SSG 与无 JavaScript 分页输出。
...
- 修复历史文章中两处 AIGC 标签、元数据或正文声明不一致，以及一处正文重复页面 H1 的问题，使全量内容能够通过统一校验。

### ✅ Verify

- 搜索命中高亮与句子感知摘要通过目标文件 Prettier、`pnpm.cmd typecheck`、`pnpm.cmd lint`、`pnpm.cmd build` 与 `git diff --check`；生产构建继续生成 70 个静态页面，并静态预渲染 `/site-search-index.json`。
...
- RSS 顶部工具区迁移通过目标文件 Prettier、`pnpm.cmd typecheck`、`pnpm.cmd lint`、`pnpm.cmd build` 与 `git diff --check`；70 个静态页面继续生成，首页、文章列表和文章详情构建产物均包含 `/rss.xml`，首页路由大小降至 9.87 kB。
- 文章分页与无限滚动加载模式通过目标文件 Prettier、`pnpm.cmd typecheck`、`pnpm.cmd lint` 与 `pnpm.cmd build`；内容门禁确认 26 篇公开文章，生产构建成功生成 74 个静态页面，并静态预渲染包含 26 篇摘要、每批 10 篇的 `/article-index.json`。构建仅保留 Browserslist 数据过期的非阻塞提示。
- 默认无限滚动偏好记忆与文章列表回到顶部通过目标文件 Prettier、`pnpm.cmd run typecheck`、`pnpm.cmd run lint`、`pnpm.cmd run build` 与 `git diff --check`；内容门禁确认 26 篇公开文章，生产构建继续成功生成 74 个静态页面。构建仅保留 Browserslist 数据过期的非阻塞提示。
