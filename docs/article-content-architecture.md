...
轻量元数据索引不提取文章标题 AST，也不长期保留全部正文源代码。详情页通过 slug 找到 sourcePath，只读取目标 MDX，再生成正文和目录标题。

全文搜索是该边界的构建期例外：生产构建只为公开文章解析 MDX AST，按 H2/H3 生成章节级搜索文档，并排除 fenced code block、ESM 和 JSX。随后与首页常驻内容、文章归档入口合并为全站搜索索引。搜索文档是可重新生成的派生数据，不取代 MDX 内容源，也不会在 Worker 请求期间读取或写入文件。

该边界保证列表、标签、RSS、sitemap 和相邻文章仍只处理摘要；只有静态搜索索引生成步骤会扫描公开正文，新增一篇文章时，内容解析量随文章总数线性增长。
...
该 JSON 包含首页与文章归档入口、公开文章摘要、文章根文档和章节级搜索文档。浏览器同时按需加载 Fuse.js，使用中文分词、字段权重和模糊匹配完成全文搜索。文章归档按文章范围去重和分页；全局搜索浮窗显示页面、文章和章节结果。没有打开全局搜索、`?q=` 或旧 `?tag=` 参数时不会请求完整索引；普通静态归档、标签页和详情页也不需要这次请求。

Fuse.js 返回的命中范围会在搜索层转换为右开区间并合并，再映射为结构化标题、章节、摘要和元数据匹配。全局浮窗与文章归档共用安全的 React `<mark>` 渲染组件，不拼接搜索结果 HTML。摘要优先选择正文或描述命中，按中英文句末和段落边界保留完整命中句，目标长度为 120–180 个中文字符、硬上限约 220；仅标题、标签或关键词命中时使用完整文档描述，并显式显示命中的元数据词。展示层不再用两行截断隐藏算法生成的上下文。

搜索索引只收录站点常驻页面内容和通过公开文章判定的文章。Cloudflare 部署时它作为静态资源分发，Worker 不负责运行时解析 MDX 或创建搜索索引。任意页面可通过顶部搜索按钮或 `Ctrl/Cmd + K` 打开原生 Dialog 浮窗。
